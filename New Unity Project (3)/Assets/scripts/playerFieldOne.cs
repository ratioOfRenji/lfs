...
public class playerFieldOne : MonoBehaviour
{
    public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldOne();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldOne;
        if(field.GetComponent<newPlanting3>().growTime > 0)
        {
           field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldOne += TimeMaster.CheckDate();
        }
        this.gameObject.transform.position = PlayerData.LocationFieldOne;

        field.transform.position = PlayerData.LocationFieldOne;
        field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldOne];
    ...
    {
        playerPrefField.SaveDataFieldOne(this);
    }
    private void Update()
    {
