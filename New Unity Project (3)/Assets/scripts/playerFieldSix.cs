...
public class playerFieldSix : MonoBehaviour
{
    public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldSix();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldSix;
       
        if (field.GetComponent<newPlanting3>().growTime > 0)
        {
            field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldSix += TimeMaster.CheckDate();
        }
        this.gameObject.transform.position = PlayerData.LocationFieldSix;
field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldSix];
        ...
        if (field.GetComponent<newPlanting3>().growTime >= 5)
        {
            //field.GetComponent<SpriteRenderer>().sprite = secondImages[PlayerData.numberFieldSix];
        }
        if (PlayerData.boolFieldSix == 1)
        {
            this.gameObject.GetComponent<dragAfterBought>().bought = true;
            this.gameObject.GetComponent<dragAfterBought>().set = true;
        }
        if (PlayerData.boolFieldSix == 0)
        {
    ...
    {
        playerPrefField.SaveDataFieldSix(this);
    }
    private void Update()
    {
