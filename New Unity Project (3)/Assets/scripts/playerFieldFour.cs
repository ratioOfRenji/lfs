...
public class playerFieldFour : MonoBehaviour
{
    public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldFour();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldFour;
       
        if (field.GetComponent<newPlanting3>().growTime > 0)
        {
            field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldFour += TimeMaster.CheckDate();
        }
        this.gameObject.transform.position = PlayerData.LocationFieldFour;
        //field.transform.position = PlayerData.LocationFieldOne;
        if (field.GetComponent<newPlanting3>().growTime < 5)
        {
          //  field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldFour];
        }
        if (field.GetComponent<newPlanting3>().growTime >= 5)
        {
            field.GetComponent<SpriteRenderer>().sprite = secondImages[PlayerData.numberFieldFour];
        }
        if (PlayerData.boolFieldFour == 1)
        {
            this.gameObject.GetComponent<dragAfterBought>().bought = true;
            this.gameObject.GetComponent<dragAfterBought>().set = true;
        }
        if (PlayerData.boolFieldFour == 0)
        {
    ...
    {
        playerPrefField.SaveDataFieldFour(this);
    }
    private void Update()
    {
