...
public class playerFieldEight : MonoBehaviour
{
    public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldEight();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldEight;
        if (field.GetComponent<newPlanting3>().growTime > 0)
        {
            field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldEight += TimeMaster.CheckDate();
        }
        this.gameObject.transform.position = PlayerData.LocationFieldEight;
 field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldEight];
        ...
        if (field.GetComponent<newPlanting3>().growTime >= 5)
        {
          //  field.GetComponent<SpriteRenderer>().sprite = secondImages[PlayerData.numberFieldEight];
        }
        if (PlayerData.boolFieldEight == 1)
        {
            this.gameObject.GetComponent<dragAfterBought>().bought = true;
            this.gameObject.GetComponent<dragAfterBought>().set = true;
        }
        if (PlayerData.boolFieldEight == 0)
        {
    ...
    {
        playerPrefField.SaveDataFieldEight(this);
    }
    private void Update()
    {
