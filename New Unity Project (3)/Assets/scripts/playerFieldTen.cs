...
public class playerFieldTen : MonoBehaviour
{
      public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldTen();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldTen;
        if (field.GetComponent<newPlanting3>().growTime > 0)
        {
            field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldTen += TimeMaster.CheckDate();
        }
        this.gameObject.transform.position = PlayerData.LocationFieldTen;
field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldTen];
        ...
        if (field.GetComponent<newPlanting3>().growTime >= 5)
        {
           // field.GetComponent<SpriteRenderer>().sprite = secondImages[PlayerData.numberFieldTen];
        }
        if (PlayerData.boolFieldTen == 1)
        {
            this.gameObject.GetComponent<dragAfterBought>().bought = true;
            this.gameObject.GetComponent<dragAfterBought>().set = true;
        }
        if (PlayerData.boolFieldTen == 0)
        {
    ...
    {
        playerPrefField.SaveDataFieldTen(this);
    }
    private void Update()
    {
