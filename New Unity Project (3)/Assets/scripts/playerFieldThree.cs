...
public class playerFieldThree : MonoBehaviour
{
    public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldThree();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldThree;
        if (field.GetComponent<newPlanting3>().growTime > 0)
        {
            field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldThree += TimeMaster.CheckDate();
        }
        this.gameObject.transform.position = PlayerData.LocationFieldThree;
field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldThree];
        ...
        if (field.GetComponent<newPlanting3>().growTime >= 5)
        {
           // field.GetComponent<SpriteRenderer>().sprite = secondImages[PlayerData.numberFieldThree];
        }
        if (PlayerData.boolFieldThree == 1)
        {
            this.gameObject.GetComponent<dragAfterBought>().bought = true;
           this.gameObject.GetComponent<dragAfterBought>().set = true;
        }
        if (PlayerData.boolFieldThree == 0)
        {
    ...
    {
        playerPrefField.SaveDataFieldThree(this);
    }
    private void Update()
    {
