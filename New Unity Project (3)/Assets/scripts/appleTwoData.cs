...
public class appleTwoData : MonoBehaviour
{
    public float timer;
   ...
   // public static float realTime;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefTrees.LoadDataAppleTwo();
        tree.GetComponent<wateringRaspberry>().timer = PlayerData.timerAppleTwo;


        tree.transform.position = PlayerData.LocationSecondtApple;
        if (PlayerData.boolAppleTwo == 1)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = true;
        }
        if (PlayerData.boolAppleTwo == 0)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = false;
        }

        if (PlayerData.spritesAppleTwo == 0)
        {
            tree.GetComponent<SpriteRenderer>().sprite = regular;
        }
        if (PlayerData.spritesAppleTwo == 1)
        {
            tree.GetComponent<SpriteRenderer>().sprite = watered;
        }
        if (PlayerData.spritesAppleTwo == 2)
        {
        ...
        if (PlayerData.boolAppleTwo == 1)
        {
            this.gameObject.GetComponent<wateringRaspberry>().timer = PlayerData.timerAppleTwo += CheckDate();
    ...
    {
        playerPrefTrees.SaveDataAppleTwo(this);
        ResetClock();
    }
    private void Update()
    {
