...
public class cherryTwoData : MonoBehaviour
{
    public float timer;
    ...
    {
       // PlayerPrefs.DeleteAll();
        PlayerData = playerPrefTrees.LoadDataCherryTwo();
        tree.GetComponent<wateringRaspberry>().timer = PlayerData.timerCherryTwo;


        tree.transform.position = PlayerData.LocationSecondCherry;
        if (PlayerData.boolCherryTwo == 1)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = true;
        }
        if (PlayerData.boolCherryTwo == 0)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = false;
        }

        if (PlayerData.spritesCherryTwo == 0)
        {
            tree.GetComponent<SpriteRenderer>().sprite = regular;
        }
        if (PlayerData.spritesCherryTwo == 1)
        {
            tree.GetComponent<SpriteRenderer>().sprite = watered;
        }
        if (PlayerData.spritesCherryTwo == 2)
        {
        ...
        if (PlayerData.boolCherryTwo == 1)
        {
            this.gameObject.GetComponent<wateringRaspberry>().timer = PlayerData.timerCherryTwo +=CheckDate();
    ...
    {
        playerPrefTrees.SaveDataCherryTwo(this); ResetClock();
    }
    private void Update()
    {
