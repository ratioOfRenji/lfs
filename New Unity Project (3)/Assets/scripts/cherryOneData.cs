...
public class cherryOneData : MonoBehaviour
{
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefTrees.LoadDataCherryOne();
        tree.GetComponent<wateringRaspberry>().timer = PlayerData.timerCherryOne;


        tree.transform.position = PlayerData.LocationFirstCherry;
        if (PlayerData.boolCherryOne == 1)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = true;
        }
        if (PlayerData.boolCherryOne == 0)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = false;
        }

        if (PlayerData.spritesCherryOne == 0)
        {
            tree.GetComponent<SpriteRenderer>().sprite = regular;
        }
        if (PlayerData.spritesCherryOne == 1)
        {
            tree.GetComponent<SpriteRenderer>().sprite = watered;
        }
        if (PlayerData.spritesCherryOne == 2)
        {
        ...
        if (this.gameObject.GetComponent<wateringRaspberry>().timer >0)
        {
            this.gameObject.GetComponent<wateringRaspberry>().timer = PlayerData.timerCherryOne += CheckDate();
    ...
    {
        playerPrefTrees.SaveDataCherryOne(this);
        ResetClock();
    }
    private void Update()
    {
