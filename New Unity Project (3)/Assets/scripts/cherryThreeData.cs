...
public class cherryThreeData : MonoBehaviour
{
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefTrees.LoadDataCherryThree();
        tree.GetComponent<wateringRaspberry>().timer = PlayerData.timerCherryThree;


        tree.transform.position = PlayerData.LocationThirdCherry;
        if (PlayerData.boolCherryThree == 1)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = true;
        }
        if (PlayerData.boolCherryThree == 0)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = false;
        }

        if (PlayerData.spritesCherryThree == 0)
        {
            tree.GetComponent<SpriteRenderer>().sprite = regular;
        }
        if (PlayerData.spritesCherryThree == 1)
        {
            tree.GetComponent<SpriteRenderer>().sprite = watered;
        }
        if (PlayerData.spritesCherryThree == 2)
        {
        ...
        if (PlayerData.boolCherryThree == 1)
        {
            this.gameObject.GetComponent<wateringRaspberry>().timer = PlayerData.timerCherryThree += TimeMaster.CheckDate();
    ...
    {
        playerPrefTrees.SaveDataCherryThree(this);
    }
    private void Update()
    {
