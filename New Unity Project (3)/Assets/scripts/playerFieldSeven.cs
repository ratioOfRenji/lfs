...
public class playerFieldSeven : MonoBehaviour
{
    public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldSeven();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldSeven;
        if (field.GetComponent<newPlanting3>().growTime > 0)
        {
            field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldSeven += TimeMaster.CheckDate();
        }
        this.gameObject.transform.position = PlayerData.LocationFieldSeven;
 field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldSeven];
        ...
        if (field.GetComponent<newPlanting3>().growTime >= 5)
        {
            //field.GetComponent<SpriteRenderer>().sprite = secondImages[PlayerData.numberFieldSeven];
        }
        if (PlayerData.boolFieldSeven == 1)
        {
            this.gameObject.GetComponent<dragAfterBought>().bought = true;
            this.gameObject.GetComponent<dragAfterBought>().set = true;
        }
        if (PlayerData.boolFieldSeven == 0)
        {
    ...
    {
        playerPrefField.SaveDataFieldSeven(this);
    }
    private void Update()
    {
