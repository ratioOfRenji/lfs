...
public class playerFieldTwelve : MonoBehaviour
{
    public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldTwelve();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldTwelve;
        if (field.GetComponent<newPlanting3>().growTime > 0)
        {
            field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldTwelve += TimeMaster.CheckDate();
        }
       // this.gameObject.transform.position = PlayerData.LocationFieldTwelve;

        field.transform.position = PlayerData.LocationFieldTwelve;
        field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldTwelve];
        ...
        if (field.GetComponent<newPlanting3>().growTime >= 5)
        {
            // field.GetComponent<SpriteRenderer>().sprite = secondImages[PlayerData.numberFieldOne];
        }
        if (PlayerData.boolFieldTwelve == 1)
        {
            this.gameObject.GetComponent<dragAfterBought>().bought = true;
            this.gameObject.GetComponent<dragAfterBought>().set = true;
        }
        if (PlayerData.boolFieldTwelve == 0)
        {
    ...
    {
        playerPrefField.SaveDataFieldTwelve(this);
    }
    private void Update()
    {
