...
public class playerFieldEleven : MonoBehaviour
{
    public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldEleven();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldEleven;
        if (field.GetComponent<newPlanting3>().growTime > 0)
        {
            field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldEleven += TimeMaster.CheckDate();
        }
        this.gameObject.transform.position = PlayerData.LocationFieldEleven;
  field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldEleven];
        ...
        if (field.GetComponent<newPlanting3>().growTime >= 5)
        {
           // field.GetComponent<SpriteRenderer>().sprite = secondImages[PlayerData.numberFieldEleven];
        }
        if (PlayerData.boolFieldEleven == 1)
        {
            this.gameObject.GetComponent<dragAfterBought>().bought = true;
            this.gameObject.GetComponent<dragAfterBought>().set = true;
        }
        if (PlayerData.boolFieldEleven == 0)
        {
    ...
    {
        playerPrefField.SaveDataFieldEleven(this);
    }
    private void Update()
    {
