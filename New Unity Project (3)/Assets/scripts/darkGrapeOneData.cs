...
public class darkGrapeOneData : MonoBehaviour
{
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefTrees.LoadDataDarkGrapeOne();
        tree.GetComponent<wateringRaspberry>().timer = PlayerData.timerDarkGrapeOne;


        tree.transform.position = PlayerData.LocationFirstDarkGrape;
        if (PlayerData.boolDarkGrapeOne == 1)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = true;
        }
        if (PlayerData.boolDarkGrapeOne == 0)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = false;
        }

        if (PlayerData.spritesDarkGrapeOne == 0)
        {
            tree.GetComponent<SpriteRenderer>().sprite = regular;
        }
        if (PlayerData.spritesDarkGrapeOne == 1)
        {
            tree.GetComponent<SpriteRenderer>().sprite = watered;
        }
        if (PlayerData.spritesDarkGrapeOne == 2)
        {
        ...
        if (PlayerData.spritesDarkGrapeOne == 1)
        {
            this.gameObject.GetComponent<wateringRaspberry>().timer = PlayerData.timerDarkGrapeOne += TimeMaster.CheckDate();
    ...
    {
        playerPrefTrees.SaveDataDarkGrapeOne(this);
    }
    private void Update()
    {
