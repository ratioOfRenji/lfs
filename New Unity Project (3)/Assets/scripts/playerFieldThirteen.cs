...
public class playerFieldThirteen : MonoBehaviour
{
    public int number;
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefField.LoadDataFieldThirteen();
        field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldThirteen;
        if (field.GetComponent<newPlanting3>().growTime > 0)
        {
            field.GetComponent<newPlanting3>().growTime = PlayerData.timerFieldThirteen += TimeMaster.CheckDate();
        }
        this.gameObject.transform.position = PlayerData.LocationFieldThirteen;
field.GetComponent<SpriteRenderer>().sprite = firstImages[PlayerData.numberFieldThirteen];
        ...
        if (field.GetComponent<newPlanting3>().growTime >= 5)
        {
           // field.GetComponent<SpriteRenderer>().sprite = secondImages[PlayerData.numberFieldThirteen];
        }
        if (PlayerData.boolFieldThirteen == 1)
        {
            this.gameObject.GetComponent<dragAfterBought>().bought = true;
            this.gameObject.GetComponent<dragAfterBought>().set = true;
        }
        if (PlayerData.boolFieldThirteen == 0)
        {
    ...
    {
        playerPrefField.SaveDataFieldThirteen(this);
    }
    private void Update()
    {
