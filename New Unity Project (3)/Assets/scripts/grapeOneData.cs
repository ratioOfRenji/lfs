...
public class grapeOneData : MonoBehaviour
{
    public float timer;
    ...
    private void OnEnable()
    {
        ...
        PlayerData = playerPrefTrees.LoadDataGrapeOne();
        tree.GetComponent<wateringRaspberry>().timer = PlayerData.timerGrapeOne;


        tree.transform.position = PlayerData.LocationFirstGrape;
        if (PlayerData.boolGrapeOne == 1)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = true;
        }
        if (PlayerData.boolGrapeOne == 0)
        {
            tree.GetComponent<wateringRaspberry>().startTimer = false;
        }

        if (PlayerData.spritesGrapeOne == 0)
        {
            tree.GetComponent<SpriteRenderer>().sprite = regular;
        }
        if (PlayerData.spritesGrapeOne == 1)
        {
            tree.GetComponent<SpriteRenderer>().sprite = watered;
        }
        if (PlayerData.spritesGrapeOne == 2)
        {
        ...
        if (PlayerData.spritesGrapeOne == 1)
        {
            this.gameObject.GetComponent<wateringRaspberry>().timer = PlayerData.timerGrapeOne += TimeMaster.CheckDate();
    ...
    {
        playerPrefTrees.SaveDataGrapeOne(this);
    }
    private void Update()
    {
